          ...
          <div className="mx-auto max-w-3xl">
            <div className="mb-6 flex flex-wrap items-center gap-x-4 gap-y-2 text-sm text-muted-foreground">
              <div className="flex items-center gap-1.5">
                ...
                {entry.publishedAt ? (
                  <time dateTime={entry.publishedAt}>{formatDateTime(entry.publishedAt)}</time>
                ) : (
                  <span>Undated</span>
                )}
              </div>
        ...
        <div className="container">
          ...
          <section className="mx-auto mt-10 max-w-3xl">
            <SocialShare
              ...
              text={entry.title || 'NABI Announcement'}
              url={`/announcements/${decodedSlug}`}
            />
          </section>
    ...
    doc: entry
      ? {
          meta: {
            ...
            image: entry.meta?.image || entry.image,
            title: entry.meta?.title || entry.title,
          },
          slug: ['announcements', decodedSlug],
        }
      : null,
    path: `/announcements/${decodedSlug}`,
