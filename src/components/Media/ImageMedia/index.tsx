...
export const ImageMedia: React.FC<MediaProps> = (props) => {
  ...
  // Tell the browser the rendered slot size so it can choose smaller responsive image variants.
  const sizes =
    sizeFromProps ||
    (fill ? '100vw' : width ? `(max-width: ${width}px) 100vw, ${width}px` : '100vw')

  return (
    <picture className={cn('block', pictureClassName)}>
      <NextImage
        alt={alt || ''}
        className={cn(!fill && 'h-auto w-full', imgClassName)}
        fill={fill}
