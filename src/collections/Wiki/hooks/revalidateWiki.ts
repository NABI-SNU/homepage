...
export const revalidateWiki: CollectionAfterChangeHook = ({ doc, previousDoc, req: { context, payload } }) => {
  ...
  const shouldRevalidatePreviousPath =
    previousDoc?._status === 'published' && (doc?._status !== 'published' || previousDoc.slug !== doc.slug)
  ...
  if (shouldRevalidatePreviousPath) {
    ...
    safeRevalidate(payload, 'previous wiki detail cache', () => revalidateTag(`wiki_${previousDoc.slug}`))
  }

  return doc
}

export const revalidateWikiDelete: CollectionAfterDeleteHook = ({ doc, req: { context, payload } }) => {
  if (isRevalidateDisabled(context)) return doc
